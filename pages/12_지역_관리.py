    ...
    if is_admin:
        ...
        for idx, region in enumerate(regions):
            with region_cols[idx % min(len(regions), 3)]:
                ...
                st.markdown(
                    f"<div style='background:#1a2a3a;border-radius:12px;padding:16px;"
                    f"border-left:4px solid #4c8df5;color:#e0e8ff'>"
                    f"<h3 style='margin:0;color:#90caf9'>🗺️ {region}</h3>"
                    f"<p style='margin:4px 0;color:#90a4ae'>매니저: {', '.join([m['name'] for m in r_managers]) or '미배정'}</p>"
                    f"<hr style='margin:8px 0;border-color:#2a3a4a'>"
                    f"<p style='color:#e0e8ff'>📋 전체 주문: <b>{len(r_orders)}건</b></p>"
                    f"<p style='color:#e0e8ff'>📦 수거: <b>{r_collection}건</b> | 🔨 철거: <b>{r_demolition}건</b></p>"
                    f"<p style='color:#e0e8ff'>⏳ 대기 중: <b>{r_pending}건</b></p>"
                    f"<p style='color:#e0e8ff'>✅ 완료: <b>{r_completed}건</b></p>"
                    f"<p style='color:#e0e8ff'>👷 담당 기사: <b>{len(r_drivers)}명</b></p>"
                    f"<p style='color:#e0e8ff'>💰 확정 매출: <b>₩{r_revenue:,}</b></p>"
                    f"<p style='font-size:12px;color:#78909c'>정산 명칭: {base_cost_label}</p>"
                    f"</div>",
        ...
        base_cost_label = labels.get("manager_base_cost", "운영비")
        st.caption(f"📌 정산 명칭 — {base_cost_label}")
    ...
    if is_admin:
        ...
        with sub_col:
            ...
            if not _coll_orders:
                ...
                _coll_filter = st.selectbox("상태 필터", ["전체", "⏳ 대기", "📍 배차", "🔄 진행중", "✅ 완료", "❌ 취소"], key="coll_filter")
        ...
        with sub_demo:
            ...
            with d2: st.metric("📋 견적 미확정", f"{len(_demo_unconfirmed)}건", delta="즉시 확정 필요" if _demo_unconfirmed else None, delta_color="inverse" if _demo_unconfirmed else "normal")
            ...
            if _demo_unconfirmed:
                st.warning(f"⚠️ 매니저 견적 미확정 {len(_demo_unconfirmed)}건")

            if not _demo_orders:
                st.info("철거 건이 없습니다.")
            else:
                _demo_filter = st.selectbox("상태 필터", ["전체", "⏳ 대기", "📍 배차", "🔄 진행중", "✅ 완료", "❌ 취소"], key="demo_filter")
        ...
        st.divider()
        st.markdown(f"#### 🎯 {active_region} 통합 실적")
...
with tab4:
    ...
    if not r_orders_settled:
        ...
        demo_incentive = len([o for o in r_orders_settled if o.get("work_type") == "철거" and o.get("manager_closed")]) * settings.get("demolition_incentive_min", 50000)

        col_s1, col_s2, col_s3, col_s4, col_s5 = st.columns(5)
        with col_s1: st.metric("총 확정 매출", f"₩{total_rev:,}")
        with col_s2: st.metric("📦 수거 수당", f"₩{coll_allow:,}")
        with col_s3: st.metric("🔨 철거 수당", f"₩{demo_allow:,}")
        with col_s4: st.metric(incentive_label, f"₩{demo_incentive:,}")
        with col_s5: st.metric(base_cost_label, f"₩{settings.get('manager_base_cost', 1500000):,}/월")
    ...
    with tab5:
        ...
        if not all_roi_orders:
            ...
            for ch in MARKETING_CHANNELS + ["미등록"]:
                ...
                _channel_stats[ch] = {
                    "유입": len(ch_orders),
                    "수거": len([o for o in ch_orders if o.get("work_type") == "수거"]),
                    "철거": len([o for o in ch_orders if o.get("work_type") == "철거"]),
                    "완료": len(ch_done),
                    "전환율": f"{ch_conv}%",
                    "확정매출": f"₩{ch_rev:,}",
                    "_rev": ch_rev,
            ...
            if _channel_stats:
                _df_ch = pd.DataFrame([
                    {"유입경로": k, "리드 수": v["유입"], "📦 수거": v["수거"], "🔨 철거": v["철거"],
                     "완료": v["완료"], "전환율": v["전환율"], "확정 매출": v["확정매출"]}
                ...
                if _untagged:
                    st.divider()
                    st.warning(f"⚠️ 유입 경로 미등록 주문 {len(_untagged)}건")
                    with st.expander(f"미등록 주문 목록 ({len(_untagged)}건)", expanded=False):
                        _utag_rows = [{"주문": f"#{o['id']}", "고객": o["customer"],
                                       "작업": "🔨 철거" if o.get("work_type") == "철거" else "📦 수거",
                                       "예약": o.get("scheduled_time", "—")[:16],
                                       "상태": status_map.get(o["status"], o["status"])} for o in _untagged]
                        ...
                        if st.button("📝 선택 주문에 유입경로 등록", key="batch_channel_save") and _batch_sel:
                            for _k in _batch_sel:
                                update_order(_batch_order_opts[_k]["id"], {"marketing_channel": _batch_ch})
            ...
            if _month_stats:
                _trend_rows = [{"월": m, "총 리드": v["총"], "📦 수거": v["수거"],
                                "🔨 철거": v["철거"], "확정 매출": f"₩{v['매출']:,}"}
